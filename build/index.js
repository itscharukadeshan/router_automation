...
app.use("/api/status/dialog", async (req, res) => {
  try {
    const status = await axios_1.default.post(
    ...
    const signal = await axios_1.default.get(
      "http://192.168.8.1/goform/goform_get_cmd_process",
      {
        params: {
          multi_data: 1,
    ...
    const unifiedData = Object.assign(
      Object.assign(Object.assign({}, status_data), signal_data),
      { uplink_rate, downlink_rate, uplink_traffic, downlink_traffic }
    );
...
app.use("/api/status/hutch", async (req, res) => {
  try {
    ...
    const signal = await axios_1.default.post(
      "http://192.168.8.2/goform/goform_get_cmd_process",
